	...
	for "_i" from 0 to count (_binConfigPatches)-1 do {
		...
		if(isClass _patchEntry) then {
			if(!((configName _patchEntry) in (call SPY_cfg_patchList))) exitWith {
                if(__GETC__(life_adminlevel)!= 0) then {diag_log str (configName _patchEntry);} else {
...
if(__GETC__(life_adminlevel) != 0) exitWith {}; //Don't run this for admins? (moved lower so can print to rpt log whats missing for admins only. less effort now required)
